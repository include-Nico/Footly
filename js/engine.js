...
export function startMatchEngine() {
    ...
    const speeds = [400, 150, 50]; // 1x, 2x, 3x
    ...
    // Resetta i gialli della SINGOLA partita
    ...
    if(Math.random() > 0.6) chanceMinutes.push(90 + randomInt(1, stoppageTime)); 

    // --- PULSANTI PRE-PARTITA ---
    ...
    document.getElementById('btn-intro-sim').onclick = () => {
        if(gameState.userTeam.coins >= 100) {
            showConfirm("Simulazione Rapida", "Vuoi saltare la partita per 💰100 monete?", () => {
                ...
                updateDashboardHeader(); // FIX: Aggiorna subito le monete
            ...
            showNotification('Fondi Insufficienti', 'Servono 💰100 monete per simulare la partita.', 'error');
        }
    };

    // --- FIX: VELOCITÀ DI SIMULAZIONE ---
    document.getElementById('btn-match-speed').onclick = (e) => {
        ...
        // Resetta l'intervallo immediatamente per applicare la nuova velocità
        if(timerInterval) {
    ...
    function applyYellowCard(p) {
        p.matchYellows = (p.matchYellows || 0) + 1;
        p.stats.yellowCards++; 
        
        if (p.matchYellows === 2) {
            p.status.suspended = 2; // 2 = Rosso preso in game (a fine match diventa 1)
            p.status.yellowCards = 0; 
            p.stats.redCards++;
            addLog(`🟥 DOPPIO GIALLO! <b>${p.name}</b> viene espulso!`, 'log-red');
            showMatchBanner('red', 'ESPULSO', `🟥 ${p.name}<br><span style="font-size:12px;">Doppio Giallo</span>`, () => { isPaused = false; renderMatchSubsList(); });
        } else if (p.status.yellowCards >= 1) {
            p.status.yellowCards++; 
            addLog(`🟨 Ammonito <b>${p.name}</b>. Era diffidato, salterà la prossima!`, 'log-yellow');
            showMatchBanner('yellow', 'AMMONIZIONE', `🟨 ${p.name}<br><span style="font-size:12px;color:var(--text-hint);">Diffidato: salterà la prossima.</span>`, () => { isPaused = false; });
        } else {
            p.status.yellowCards++; 
    ...
    function applyRedCard(p) {
        p.status.suspended = 2; 
        p.status.yellowCards = 0;
        p.stats.redCards++;
        addLog(`🟥 ROSSO DIRETTO! <b>${p.name}</b> finisce negli spogliatoi!`, 'log-red');
        showMatchBanner('red', 'ESPULSO', `🟥 ${p.name}`, () => { isPaused = false; renderMatchSubsList(); });
    }
        ...
        if(Math.random() < 0.01) {
            ...
            if(active.length > 0) {
                ...
                if(rand > 0.85) { 
                    ...
                    showMatchBanner('injury', 'INFORTUNIO', `🤕 ${p.name} deve uscire!`, () => { isPaused = false; renderMatchSubsList(); });
                } else { 
                    applyYellowCard(p);
                }
            }
    ...
    function triggerMatchEvent() {
        ...
        if (randEvent < 0.4) {
            ...
            addEventButton(`Fallo Tattico (Rischio Cartellino)`, () => {
                ...
                if(r < 0.3) { applyRedCard(pDef); } 
                else { applyYellowCard(pDef); }
            });
    ...
    function renderMatchSubsList() {
        ...
        currentF.pos.forEach((slot, idx) => {
            ...
            if(p) {
                ...
                // MOSTRA QUADRATINO GIALLO SE DIFFIDATO (O HA UN GIALLO NEL MATCH)
                ...
                else if (p.status.yellowCards === 1) warningHTML += `<div class="oop-warning" style="right: auto; left: -8px; background: var(--gold); color: #000;" title="Diffidato / Ammonito"><i class="fas fa-square"></i></div>`;
    ...
    function endGame() {
        ...
        gameState.userTeam.players.forEach(p => {
            // Sconta Squalifiche Passate / Cartellini Rossi
            if(p.status.suspended > 0) {
                p.status.suspended--;
            }
            // Sconta Infortuni
            if(p.status.injured > 0 && !p.isStarter) {
                p.status.injured--;
            }
            
            // Somma di ammonizioni (diffidato + giallo) diventa squalifica per la PROSSIMA
            if (p.status.yellowCards >= 2 && p.status.suspended === 0) {
        ...
        updateDashboardHeader(); // FIX: Aggiorna l'UI in alto
...
function renderSquad() {
    ...
    currentF.pos.forEach((slot, idx) => {
        ...
        if (p) {
            ...
            // MOSTRA QUADRATINO GIALLO SE DIFFIDATO O AMMONITO
            let warningHTML = isOOP ? `<div class="oop-warning" title="Fuori Ruolo!"><i class="fas fa-exclamation"></i></div>` : '';
