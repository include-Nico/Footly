...
import { randomName, processEndOfSeason, generatePlayer } from './players.js'; 
...
function renderHome() {
    ...
    // Classifica combinando la squadra Utente + CPU della stessa Divisione
    if (gameState.world && gameState.world[gameState.userTeam.division]) {
        ...
        standings.push({
            name: gameState.userTeam.name, isUser: true,
...
function handleEndSeason() {
    ...
    showConfirm("Stagione Conclusa!", msg, () => {
        renderHome();
    }, "Vai alla nuova Stagione", false);
}
...
function renderSquad() {
    ...
    formSelect.onchange = (e) => {
        ...
        selectedPlayerId = null; saveGame(); renderSquad(); 
    };
    ...
    if(isSvincoloMode) {
        ...
        let totalGain = 0;
        selectedForRelease.forEach(id => { let p = gameState.userTeam.players.find(pl => pl.id === id); if(p) totalGain += Math.floor(p.value * 0.9); });
    ...
    btnConfirmSvincolo.onclick = () => {
        ...
        showConfirm("Svincolo Multiplo", `Svincolando ${selectedForRelease.size} giocatori riclaverai 💰${totalGain.toLocaleString()}. Procedere?`, () => {
...
function renderMarket() {
    ...
    searchBtn.onclick = () => {
        ...
        // CERCA IN TUTTE LE 3 DIVISIONI DEL MONDO!
        let allPlayers = [];
        if(gameState.world) {
            [1, 2, 3].forEach(div => {
                if(gameState.world[div]) {
                    gameState.world[div].forEach(team => {
                        team.roster.forEach(p => { 
                            ...
                            p.divLevel = div; // Tracciamo da che divisione viene
                            allPlayers.push(p); 
                        });
                    });
                }
